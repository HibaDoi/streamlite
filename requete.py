    ...
    for index, row in gdf.iterrows():
        try:
            ...
            folium.CircleMarker(
                ...
                fill=True,
                fill_color='blue',
                popup=f"Hotel: {row['Hotel name']}, Rank: {row['Rank ']}, Date: {row['Date']}"
            ).add_to(carte)

        except Exception as e:
            st.warning(f"Error extracting coordinates for hotel {row['Nom_Commun']}: {e}")
...
def main():
    ...
    try:
        ...
        gdf_all = load_hotel_data("waaaa.geoparquet",
                                  min_latitude, max_latitude, min_longitude, max_longitude)

        # Exclude specific columns from the attribute selection, including "geometry"
        excluded_columns = ["FID","OBJECTID","Code_Commu","Nom_Commun","Nom_Comm_1","Code_Provi","CODE_REGIO","Code_Com_O","Nom_Com_Ol","Shape__Are","Shape__Len","Shape__A_1","Shape__L_1","ORIG_FID","latitude","longitude","OBJECTID_1"]
